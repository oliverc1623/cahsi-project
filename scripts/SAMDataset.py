...
class SAMDataset(data.Dataset):
    ...
    def __init__(
        self, folder_path, processor, image_transform=None, mask_transform=None
    ):
        ...
        for img_path in self.img_files:
            self.mask_files.append(
                os.path.join(
                    folder_path, "Ground-truths", f"mask_{os.path.basename(img_path)}"
                )
            )
    ...
    def __getitem__(self, index):
        ...
        inputs = self.processor(
            data, input_boxes=[[prompt]], return_tensors="pt", do_rescale=False
        )
